...
class PrivateView extends View{
	...
	public function makeMotoModifPage($id,Moto $moto){
		...
		$this->content = '<div class="middle"><h2>Modifier votre Moto </h2>
							<form class="modif" action="'. $this->router->updateModifiedMoto($id).'" enctype="multipart/form-data" method="POST">
						  		<p>Modele </p> <input type="text" name='.MotoBuilder::MODELE_REF.' value="'.$moto->getModele().'"/>
						  		<p>Marque </p>
						  		<select name="'.MotoBuilder::MARQUE_REF.'">
	...
	/*
	 * Remplis le contenu avec un formulaire permettant à l'utilisateur de modifier son Nom,Login,Password.
	 */
	public function makeAccountPage(){
		$this->title = "Account";
		$this->content = '<div class="middle"><p>Compte de '.$_SESSION['user']->getName().'</p>';
		$this->content .= '<form action="'.$this->router->updateUser().'" method="POST">
								<input type="hidden" name="id" value="'.$_SESSION['user']->getId().'" />
								<p>Name </p> <input type="text" name="name" value="'.$_SESSION['user']->getName().'"/>
								<p>Login </p> <input type="text" name="login" value="'.$_SESSION['user']->getlog().'"/>
								<p>Nouveau MDP </p> <input type="password" name="password"/>
								<p>Confirmer MDP </p> <input type="password" name="password2"/>
								<br /><input type="submit" value="Modifier">
						   </form></div>';
	}
	...
	public function getMenu(){
		$menu = array(
			...
			"Compte"	=> $this->router->getAccountUrl(),
	...
	public function displayUserUpdateSuccess(){
		$this->router->POSTredirect($this->router->getAccountUrl(),"Update Success");
	}

	public function displayUserUpdateError(){
		$this->router->POSTredirect($this->router->getAccountUrl(),"Update ERROR");
	}
}
